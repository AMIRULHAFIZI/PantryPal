...
<div class="fixed bottom-0 left-0 w-full bg-slate-900 border-t border-slate-800 sm:hidden z-[9999] px-6 py-3 flex justify-between items-center shadow-[0_-4px_20px_rgba(0,0,0,0.5)]">
    ...
    <a href="{{ route('smart-scan.index') }}" class="relative -top-5 flex flex-col items-center justify-center">
        <div class="p-4 rounded-full text-white transform transition-all active:scale-95 border-4 border-slate-900
            {{ request()->routeIs('smart-scan.*')
                ? 'bg-emerald-400 shadow-lg shadow-emerald-400/60 ring-2 ring-emerald-300/70'
    ...
    <!-- FAQ -->
    <a href="{{ route('faq.index') }}" class="flex flex-col items-center justify-center w-16 transition-colors duration-200 {{ request()->routeIs('faq.*') ? 'text-emerald-400' : 'text-slate-500 hover:text-slate-300' }}">
        <svg class="h-6 w-6 mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
